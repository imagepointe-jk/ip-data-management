...
export function createOrderDiff(
  ...
  return {
    ...
    lineItemsCount: diff(
      initialOrder.lineItems.length,
      initialOrder.lineItems.length - lineItemIdsRemoved.length,
    ),
    shipping: {
      firstName: diff(
...
//currently this returns true ONLY if something changed with the line items, per a feature request.
...
export function shouldSendOrderUpdatedEmails(
  ...
  //add diffStatuses from the order diff to this array to take additional changes into account when deciding whether to send the email
  const relevantDiffs = [
    // diff.customerNote,
    // diff.shippingMethod,
  ...
  const anyRelevantChanges = relevantDiffs.some((diff) => diff === "changed");
